...
describe('createSubscriptionRecovery', () => {
  ...
  // account, a gateway secret mismatch) — without a cap that is an infinite
  // refresh/reconnect spin against the API.
  it('gives up after one attempt instead of spinning', async () => {
    const refresh = vi.fn(async () => true);
    const reconnect = vi.fn();
    const recovery = createSubscriptionRecovery({ refresh, reconnect });
