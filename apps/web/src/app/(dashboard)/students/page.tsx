...
export default function StudentsPage() {
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <Button type="button" variant="outline" size="sm" asChild>
            <Link href={`/students/${row.original.id}/edit`}>Sửa</Link>
          </Button>
          <Button
            type="button"
            variant="destructive"
            size="sm"
            onClick={() => {
              if (window.confirm(`Xóa SV "${row.original.studentCode}"?`)) {
  ...
  return (
    <PageShell>
      <PageHeader
        title="Sinh viên"
        description="Danh sách sinh viên trong hệ thống."
        actions={
          <Button asChild>
            <Link href="/students/new">Thêm mới</Link>
          </Button>
        }
      />
      <Input
