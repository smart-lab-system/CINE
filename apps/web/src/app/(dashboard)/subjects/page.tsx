...
export default function SubjectsPage() {
  ...
  const columns = [
    ...
    columnHelper.accessor('credits', { header: 'Tín chỉ' }),
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <Button type="button" variant="outline" size="sm" asChild>
            <Link href={`/subjects/${row.original.id}/edit`}>Sửa</Link>
          </Button>
          <Button
            type="button"
            variant="destructive"
            size="sm"
            onClick={() => {
              if (window.confirm(`Xóa môn "${row.original.code}"?`)) {
  ...
  return (
    <PageShell>
      <PageHeader
        title="Môn học"
        description="Danh mục môn học."
        actions={
          <Button asChild>
            <Link href="/subjects/new">Thêm mới</Link>
          </Button>
        }
      />
      <Input
